...
def et_param(k):
    ...
    n_estimators = [i for i in range(200,400,10)]
    max_depth = [5,6,7,8]
    #class_weight = [i/100.0 for i in range(300,600,30)]
    ...
    model = et_model(n_estimators[k],criterion[k%2],max_depth[k%4],max_features[k],min_samples_leaf[k])#
    return model

def et_model(n_estimators=300,criterion='entropy',max_depth=6,max_features=0.5,min_samples_leaf=50):#,class_weight=6
    model=	ExtraTreesClassifier(n_estimators=n_estimators,n_jobs=20,criterion=criterion,min_samples_leaf=min_samples_leaf,class_weight='balanced',
...
def main():
    ...
    filename="H:\\ET/model/main_et_A_20170220" # nam prefix
    ...
    y, X = train.label.values,train[fea_set[range(0,len(fea_set),2)]].values
    ...
    mean_auc = 0.0
    mean_ks = 0.0
    bagging=3 # number of models trained with different param
